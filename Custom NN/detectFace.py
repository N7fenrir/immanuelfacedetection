...
from constants import YOLO_TARGET, YOLO_SIZE, CORRECTOR_SIZE, MODELS_PATH
...
class FaceDetector(object):
    ...
    def __init__(self):
        self.load_model(os.path.join(MODELS_PATH, "FaceSet.pb"))
        self.load_aux_vars()
    ...
    def predict(self, frame, thresh = 0.775):
        ...
        bboxes = self._normax_suppression(bboxes)
    ...
    def _normax_suppression(self,  bboxes, thresh=0.2):
        SUPPRESSED = 1
        NON_SUPPRESSED = 2
        ...
        for i in range(N):
            ...
            for j in range(i+1, N):
                ...
                metric = boundingBox(bboxes[i], bboxes[j])
                if metric > thresh:
